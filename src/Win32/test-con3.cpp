...
#if defined(_MSC_VER) && !defined(USE_ANSI_RC)
...
#ifdef USE_ANSI_RC
    // Expect the RC file to be in ANSI only
    len = LoadString(hInstance, IDS_APP_TITLE, szTitle, MAX_LOADSTRING);
    len = LoadString(hInstance, IDC_WIN32PROJECT1, szWindowClass, MAX_LOADSTRING);
#else
    // Expect the RC file to be in UNICODE (UCS2) as generated by the MSVC140 IDE
